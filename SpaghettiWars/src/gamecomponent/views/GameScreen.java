...
public class GameScreen implements IGameScreen{
    ...
    public void render() {
        ...
        if(model.getPlayer().getPowerUp() != null)
        	batch.draw(model.getPlayer().getPowerUp().getSprite(), camera.position.x-camera.viewportWidth/2 + 70, camera.position.y-camera.viewportHeight/2 + 60, model.getPlayer().getPowerUp().getSprite().getOriginX(), model.getPlayer().getPowerUp().getSprite().getOriginY(), model.getPlayer().getPowerUp().getSprite().getWidth(), model.getPlayer().getPowerUp().getSprite().getHeight(), 2, 2, model.getPlayer().getPowerUp().getSprite().getRotation());
